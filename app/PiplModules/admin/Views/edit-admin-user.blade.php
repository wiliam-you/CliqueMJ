...
<title>Update Sub-admin User Profile</title>
...
<div class="page-content-wrapper">
    <div class="page-content">
        ...
        <ul class="page-breadcrumb breadcrumb">
            ...
            <li>
                <a href="{{url('admin/admin-users')}}">Manage Sub-admin users</a>
                <i class="fa fa-circle"></i>
            </li>
            <li>
                <a href="javascript:void(0);">Update Sub-admin User Profile</a>
            </li>
        </ul>
                        ...
                        <div class="portlet-body">
                            <div class="tab-content">
                                ...
                                <div class="tab-pane @if(!($errors->has('email') || $errors->has('confirm_email')|| $errors->has('current_password')|| $errors->has('new_password') || $errors->has('confirm_password') || session('password-update-fail'))) active @endif" id="tab_1_1">
                                    <form name="update_profile"  id="update_profile" role="form" method="POST" action="{{ url('/admin/update-admin-user/'.$user_info->id)}}">
                                        {!! csrf_field() !!}
                                        ...
                                        <div class="form-group{{ $errors->has('gender') ? ' has-error' : '' }}">
                                         ...
                                         <select class="form-control" id="role" name="role" >
                                            @foreach($roles as $role)
                                                <option value="{{$role->id}}" @if($user_info->hasRole($role->slug)) selected @endif>{{$role->name}}</option>
                                            @endforeach
                                            </select>
                                            @if ($errors->has('role'))
                                                            <span class="help-block">
                                ...
                                <div class="tab-pane @if($errors->has('email') || $errors->has('confirm_email'))  active @endif" id="tab_1_3">
                                    <form name="update_email"  id="update_email" role="form" method="POST" action="{{ url('/admin/update-admin-user-email/'.$user_info->id) }}">
                                        {!! csrf_field() !!}
                                ...
                                <div class="tab-pane @if($errors->has('current_password')|| $errors->has('new_password') || $errors->has('confirm_password') || session('password-update-fail')) active @endif" id="tab_1_2">
                                    <form name="update_password"  id="update_password" role="form" method="POST" action="{{ url('/admin/update-admin-user-password/'.$user_info->id) }}">
                                        {!! csrf_field() !!}
